function submitIssue(e) {
  ...
  const priority = getInputValue("issuePriority");
  ...
  if (description.length == 0 || assignedTo.length == 0) {
    ...
    const issue = { id, description, priority, severity, assignedTo, status };
    let issues = [];
...
const fetchIssues = () => {
  ...
  for (var i = 0; i < issues.length; i++) {
    const { id, description, priority, severity, assignedTo, status } =
      issues[i];
    ...
    issuesList.innerHTML += `<div class="well">
                              ...
                              <h3> ${description} </h3>
                              <p><span class="glyphicon glyphicon-signal"></span> ${priority}</p>
