...
class LinkedList {
  ...
  removeLast() {
    ...
    while (node && node.next && node.next.next) {
      // will enter here only if `size` is greater than 2
      node = node.next;
      entered = 1;
    }
  ...
  getMiddle(getFirstMiddle = true, onlyData = true) {
    /* Get first middle is for those cases where linked list is of even size and have two middles */
    let first = this.head;
  ...
  getFromLast(index, onlyData = true) {
    ...
    let counter = 0;
    let enteredInLoop = 0;
    const checkValidity = (nodeIndex) =>
      Array.from(Array(index + 1)).every(
        (_, index) => this.getAt(nodeIndex + index + 1) !== "NONE"
      );
  ...
  search(value, returnFullInfo = false, indexBasedSearching = false) {
    ...
    while (node) {
      if (node.data === value) {
        message = `Found at ${
          indexBasedSearching
            ? index + getSuffix(index) + " index."
            : index + 1 + getSuffix(index + 1) + " position."
        }`;
        return returnFullInfo
          ? { message, foundAt: indexBasedSearching ? index : index + 1 }
          : message;
      }
      index++;
      node = node.next;
  ...
  reverseBetweenRange(left, right) {
    ...
    leftIndexNodePredecessor =
      leftIndexNodePredecessor === "NONE" ? null : leftIndexNodePredecessor;

    let rightIndexNode = this.getAt(right, false);
    let rightIndexNodeSuccessor = this.getAt(right + 1, false);
    rightIndexNodeSuccessor =
      rightIndexNodeSuccessor === "NONE" ? null : rightIndexNodeSuccessor;

    if (leftIndexNode !== "NONE" && rightIndexNode !== "NONE") {
      /* Split linked list into three parts :- first + sub-linked list to be reversed + last  */
      if (
        leftIndexNodePredecessor !== "NONE" &&
        leftIndexNodePredecessor instanceof Node
      )
        leftIndexNodePredecessor.next = null;
      ...
      if (
        ...
        leftIndexNodePredecessor.next = leftIndexNode;
  ...
  mergeAndUpdate(linkedListA, linkedListB) {
    if (
      linkedListA instanceof LinkedList &&
      linkedListB instanceof LinkedList
    ) {
